# def hash_key( key, m):
#     return key % m

# m = 7

# print(f'The hash value for 3 is {hash_key(3,m)}')
# print(f'The hash value for 2 is {hash_key(2,m)}')
# print(f'The hash value for 9 is {hash_key(9,m)}')
# print(f'The hash value for 11 is {hash_key(11,m)}')
# print(f'The hash value for 7 is {hash_key(7,m)}')


#Python comes with a built-in data type called Dictionary. 
#A dictionary is an example of a hash table. 
# It stores values using a pair of keys and values. 
# The hash values are automatically generated for us, and any collisions are resolved for us in the background.

# mahasiswa = {
#     'nama': 'Budi',
#     'umur': 20,
#     'prodi': 'Sistem Informasi'
# }

# print (f"Nama mahasiswa adalah {mahasiswa['nama']}")
# mahasiswa['prodi'] = 'Informatika'
# print (f"Prodi {mahasiswa['nama']} adalah {mahasiswa['prodi']}")
# mahasiswa.clear()

# print (mahasiswa)